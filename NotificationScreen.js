...
//             : null;
//         if (latestNotifications !== latestNotification) {
...
//   const handleNewNotification = (latestNotification) => {
//     if (latestNotification && latestNotification.sharedId !== lastResponseTime) {
//       setLastResponseTime(latestNotification.sharedId);
//       setLastNotifications(latestNotification);
//       setUserId(userId);
...
const NotificationScreen = () => {
  ...
  const fetchNotifications = useCallback(() => {
    if (socket) {
      ...
      socket.on("getLocation", (data) => {
        const latestNotifications = data.slice(-9);
        setNotifications(latestNotifications);
        setStoredNotifications(latestNotifications); // Update stored notifications
        const latestNotification =
          latestNotifications.length > 0
            ? latestNotifications[latestNotifications.length - 1]
            : null;
  ...
  }, [socket, userId, setNotifications, setStoredNotifications]);
